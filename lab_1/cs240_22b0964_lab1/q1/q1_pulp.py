...
def pulp_solver(A_matrix: numpy.array, c: numpy.array, b: numpy.array) -> (numpy.array, float):
    ...
    ################################################################
    # %% Student Code Start
    solution = pulp.LpProblem("Simplex",pulp.LpMaximize)
    matrix_size = len(c)

    var = [pulp.LpVariable(f'x_{i}', lowBound = 0) for i in range(matrix_size)]

    function = pulp.lpSum([c[i]*var[i] for i in range(matrix_size)])
    solution += function

    for i in range(matrix_size):
        solution += pulp.lpSum([A_matrix[i][j]*var[j] for j in range(matrix_size)]) <= b[i]

    solution.solve(pulp.PULP_CBC_CMD(msg=0))
    for i in range(matrix_size):
        x[i] = pulp.value(var[i])
    opt_val = solution.objective.value()
    # Implement here
    # %% Student Code End
    ################################################################
