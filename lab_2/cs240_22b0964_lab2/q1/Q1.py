...
def w_closed_form(X, Y):
    ...
    w_closed = torch.round(w_closed*(10**4))
    w_closed = w_closed/(10**4)
...
def l2_loss(X, Y, w):
    ...
    loss = torch.sum((Y-torch.matmul(X,w))**2)/len(X)
    #print(f"L@ Loss:{loss}")
...
def l2_loss_derivative(X, Y, w):
    ...
    # derivative = torch.zeros(d,1)

    # for i in range(d):
    #     for j in range(n):
    #         derivative[i] += (-2/n)*(Y[j] - torch.matmul(X,w)[j])*X[j][i]

    derivative = (2/n)*(torch.matmul(torch.matmul(X.T,X),w) - torch.matmul(X.T,Y))
    # Student code end
