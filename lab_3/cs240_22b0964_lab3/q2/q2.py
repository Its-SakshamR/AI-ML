...
def lasso_loss(X, Y, w, lambd):
    ...
    loss = torch.sum((Y-torch.matmul(X,w))**2)/len(X) + torch.sum(lambd*torch.abs(w))
    ### YOUR CODE ENDS HERE ###
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    ### YOUR CODE BEGINS HERE ###
    n = len(X)
    derivative = (2/n)*(torch.matmul(torch.matmul(X.T,X),w) - torch.matmul(X.T,Y)) + lambd*torch.sign(w)
    ### YOUR CODE ENDS HERE ###
