...
def gini_index(classes):
    ...
    gini_impurity_value = 1-np.sum(probabilities*probabilities)
...
def entropy(classes):
    ...
    # Get unique labels and their counts in the target dataset
    unique_vals, counts = np.unique(classes, return_counts = True)
...
def information_gain(examples, classes, attr):
    ...
    attr_values, attr_counts = np.unique(examples[: , attr], return_counts = True)

    # Calculate the entropy of the entire dataset before splitting
    entropy_parent = entropy(classes)

    ## Calculate the entropy after splitting for all unique values of the attribute
    split_entropy = 0
    for value, count in zip(attr_values, attr_counts):
        ...
        split_entropy += (count / len(classes)) * subset_entropy

    # Duplicate the classes vertically, to match each example

    # Calculate and return the information gain
    information_gain_value = entropy_parent - split_entropy
...
def gini_gain(example, classes, attr):
    ...
    # Get unique values and their counts for the specified attribute
    attr_values, attr_counts = np.unique(example[: , attr], return_counts = True)

    # Calculate the gini index of the entire dataset before splitting
    gini_index_parent = gini_index(classes)
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            for attribute in attributes:
                inf_gain = information_gain(features,classes,attribute)
            ...
            # print(best_attr)
            unique_values = np.unique(features[: , best_attr])
            # print(features)
            # print(unique_values)
            ...
            rem_attr = [attr for attr in attributes if attr != best_attr]

            # Split the dataset based on the unique values of the best attribute.
            for value in unique_values:
                subset_indices = features[:, best_attr] == value
                # print(subset_indices)
                subset_features = features[subset_indices,:]
                # print(subset_features)
                subset_classes = classes[subset_indices]
                # print(len(classes)*len(features))
                children[value] = self.__build_tree__(subset_features, subset_classes, rem_attr)

            # Determine the default prediction
            unique_classes, counts = np.unique(classes, return_counts=True)
            default_for_empty = unique_classes[np.argmax(counts)]
