...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        ### Student Code start
        self.tree = DecisionTreeClassifier(random_state=42, criterion=criterion)
        self.tree.fit(self.X_train, self.Y_train)

        y__train = self.tree.predict(self.X_train)
        y__test = self.tree.predict(self.X_test)

        train_accuracy = metrics.accuracy_score(self.Y_train, y__train)
        test_accuracy = metrics.accuracy_score(self.Y_test, y__test)

        results = {"train_accuracy": train_accuracy, "test_accuracy": test_accuracy}
    ...
    def train_model_regularised(self, criterion="entropy"):
        ...
        for max_depth in range(1, 11):
            for min_samples_split in range(3, 31, 3):
                for min_samples_leaf in range(1, 11):
                    tree = DecisionTreeClassifier(max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, random_state=42, criterion=criterion)
                    tree.fit(self.X_train, self.Y_train)
                    
                    y__train = tree.predict(self.X_train)
                    y__test = tree.predict(self.X_test)
