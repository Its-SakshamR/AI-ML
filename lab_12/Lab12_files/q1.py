...
def check(array, target, forstrat, num_players, strategy, util_matrix):
    ...
    for i in range(len(getindex)):
        # print(f"Comparing elements: {util_matrix[tuple(getindex[i])][target]} and {util_matrix[tuple(forstrat)][target]}")
...
def find_strongly_dominant_eq(gamedata, playerno, totalplayer, topplayer, strategies,
    ...
    if len(totalplayer) >= 1:  # If there are remaining players to consider
        ...
        ##########        TASK 1           ###############
        for strat in range(strategies[playerno]):
            temparray=strategyarr[:]
            temparray.insert(playerno,strat)
            payoff = gamedata[sel_index(playerno,temparray,multiplier,num_players)]
            if(payoff>max_payoff):
                other_payoffs.append(max_payoff)
                max_payoff=payoff
                max_index=strat
            else:
    ...
    if len(totalplayer) >= 1:
        ...
        for strat in range(strategies[playerno]):
            ...
            if(payoff>max_payoff):
                ...
                currindex.append(strat)
        if(eqindex[0]!=-1):
            eqindex=intersect(eqindex,currindex)
        else:
            eqindex=currindex
...
def psne_gen(num_players, strategy, util_matrix):
    ...
    ##########     TASK 3         ###############
    perm = make_allpermut(num_players,strategy)
    for stratcomb in perm:
        psne = True
        for player in range(num_players):
            for opponent in range(strategy[player]):
                if util_matrix[tuple(stratcomb)][player] < util_matrix[tuple(list(stratcomb[:player]) + [opponent] + list(stratcomb[player + 1:]))][player]:
                    psne =False
                    break
            if not psne:
                break
        if psne:
            psnelist.append(stratcomb)
