...
class NeuralNetwork:
    ...
    def cost(self, training_data):
        ...
        for x, y in training_data:
            ...
            cost += 0.5 * np.linalg.norm(output - y) ** 2
    ...
    def forward_pass(self, x):
        ...
        # Here
        # 1. Calculate z and a for each layer
        # 2. Use the sigmoid function to calculate activations
        # 3. Return the final output
        z = []
        a = []
        a.append(x)
        for i in range(self.num_layers - 1):
            z.append(np.dot(self.weights[i].T, a[i]) + self.biases[i])
    ...
    def mini_batch_GD(self, training_data, epochs, mini_batch_size, eta, test_data):
        ...
        for j in range(epochs):
            ...
            for mini_batch in mini_batches:
                ...
                #  The gradients are calculated for each layer and stored in lists dC_db and dC_dw
                dC_db = [np.zeros(b.shape) for b in self.biases]
                dC_dw = [np.zeros(w.shape) for w in self.weights]
                for x, y in mini_batch:
                    dC_db_temp, dC_dw_temp = self.back_propagation(x, y)
                    dC_db = [dc + dcb for dc, dcb in zip(dC_db, dC_db_temp)]
                    dC_dw = [dw + dcw for dw, dcw in zip(dC_dw, dC_dw_temp)]
 
                # TODO (2) Update biases and weights using the computed gradients
                self.biases = [b - (eta / len(mini_batch)) * db for b, db in zip(self.biases, dC_db)]
                self.weights = [w - (eta / len(mini_batch)) * dw for w, dw in zip(self.weights, dC_dw)]
    ...
    def back_propagation(self, x, y):
        ...
        for l in range(2, self.num_layers):
            z_temp = z[-l]
            sp = self.sigmoid_derivative(z_temp)
